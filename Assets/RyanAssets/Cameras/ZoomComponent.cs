...
namespace RyanAssets.Cameras
{
    public class ZoomComponent : MonoBehaviour
    {
        ...
        [SerializeField, Min(0f), Tooltip("Base zoom units per second while E (in) or Q (out) is held. Keyboard zoom is 5x the configured zoom sensitivity.")]
            ...
            bool immediate = false)
        {
            ...
            CharacterControls controls = InputService.characterControls;
            if (controls == null || !controls.IsPointerOverScrollRect())
            {
                SetZoom(DesiredZoom + scrollWheel.ReadValue<float>() * sensitivity +
                    (controls?.KeyboardZoom ?? 0f) * keyboardZoomSpeed * sensitivity * 5f * Time.unscaledDeltaTime);
            }
